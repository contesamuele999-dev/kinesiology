...
async function stats(env, req) {
  ...
  const res = await env.DB.prepare(
    `SELECT space, COUNT(*) AS righe, COALESCE(SUM(LENGTH(ct)), 0) AS byte,
            SUM(deleted) AS lapidi, MAX(updated_at) AS ultimo
     FROM records GROUP BY space ORDER BY byte DESC LIMIT 200`).all();
  const rows = res.results || [];
  return {
    spazi: rows.length,
    byteTotali: rows.reduce((s, r) => s + r.byte, 0),
    dettaglio: rows.map((r) => ({ spazio: r.space.slice(0, 12), righe: r.righe, byte: r.byte, lapidi: r.lapidi, ultimo: r.ultimo }))
  };
}
