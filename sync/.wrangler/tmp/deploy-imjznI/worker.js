...
var bad = /* @__PURE__ */ __name((m) => Object.assign(new Error(m), { status: 400 }), "bad");
async function sync(env, space, body, invite) {
  ...
  if (changes.length) {
    ...
    if (use.n + changes.length > MAX_ROWS || use.b + inBytes > MAX_BYTES)
      throw Object.assign(new Error("spazio pieno: fai pulizia o passa a un piano pi\xF9 grande"), { status: 507 });
    await env.DB.prepare("INSERT INTO spaces(space, seq) VALUES(?, 0) ON CONFLICT(space) DO NOTHING").bind(space).run();
    ...
    const batch = changes.map((c) => {
      ...
      return stmt.bind(
        ...
        String(c.updatedAt),
        seq,
        c.deleted ? "" : String(c.iv || ""),
        c.deleted ? "" : String(c.ct || ""),
        c.deleted ? 1 : 0
      );
    });
  ...
  const out = rows.map((r) => r.deleted ? { store: r.store, id: r.id, updatedAt: r.updated_at, deleted: 1 } : { store: r.store, id: r.id, updatedAt: r.updated_at, iv: r.iv, ct: r.ct });
  return {
...
async function stats(env, req) {
  ...
  const res = await env.DB.prepare(
    `SELECT s.space AS space, COUNT(r.id) AS righe, COALESCE(SUM(LENGTH(r.ct)), 0) AS byte,
     ...
     GROUP BY s.space ORDER BY byte DESC LIMIT 200`
  ).all();
  const rows = res.results || [];
  return {
    spazi: rows.length,
    ...
    dettaglio: rows.map((r) => ({
      spazio: r.space.slice(0, 12),
      righe: r.righe,
      byte: r.byte,
      lapidi: r.lapidi,
      ultimo: r.ultimo,
      termini: r.ver ? r.ver + " del " + r.accettato : "non accettati"
    }))
  };
}
__name(stats, "stats");
